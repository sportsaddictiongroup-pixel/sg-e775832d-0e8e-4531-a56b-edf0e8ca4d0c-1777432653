...
export default function TalentDirectoryPage() {
  ...
  if (isLoading) {
    return (
      <main className="min-h-screen flex items-center justify-center bg-background">
        <p className="text-sm text-muted-foreground animate-pulse">Loading Directory...</p>
      </main>
    );
  }
  ...
  return (
    <>
      <SEO title="Talent Directory" description="View and manage your registered talents." />
      <main className="min-h-screen bg-background text-foreground px-4 pt-8 pb-8 md:py-8 overflow-x-hidden">
        <div className="mx-auto w-full max-w-7xl space-y-6">
          ...
          {profile && <TalentManagement profile={profile} mode="directory" />}
        </div>
      </main>
    </>
  );
